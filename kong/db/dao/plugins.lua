...
local function has_at_least_one_common_protocol_with_route(plugin, route)
...
local function has_at_least_one_compatible_route_in_service(self, plugin, service_pk)
  for route, err, err_t in self.db.routes:each_for_service(service_pk) do
    if not route then
      return nil, err, err_t
    end

    if has_at_least_one_common_protocol_with_route(plugin, route) then
      return true
    end
  end
end
...
local function check_protocols_match(self, plugin)
  ...
  if type(plugin.route) == "table" then
    ...
    if route and not has_at_least_one_common_protocol_with_route(plugin, route) then
  ...
  if type(plugin.service) == "table" then
    if not has_at_least_one_compatible_route_in_service(self, plugin, plugin.service) then
      local err_t = self.errors:schema_violation({
